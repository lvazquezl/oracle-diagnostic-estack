...
18/18 agentes obligatorios (sección 7 del prompt maestro), todos `status: active`, conformes al [Agent Contract](../docs/CONTRACTS.md#agent-contract). `oracle-discovery-analyst` y `oracle-dba-analyst` están en `v2.0.0` (Fase 2 — Oracle Core, desarrollo profundo con estructura de carpeta `agents/<id>/AGENT.md`); `oracle-performance-analyst` está en `v4.0.0` (Fase 3 Completion & Portability Hardening — contrato estructurado completo); `oracle-rac-analyst`/`oracle-asm-storage-analyst`/`oracle-network-analyst` están en `v2.0.0` (Fase 4 — RAC/GI/ASM/Network, mismo contrato estructurado completo); `oracle-dataguard-analyst` está en `v2.0.0` (Fase 5 — Data Guard, mismo contrato estructurado completo: `AGENT.md` + `manifest.yaml` + `routing.yaml` + `context-policy.yaml` + `collaboration.yaml` + `output-schema.yaml` + `tests/` + `CHANGELOG.md`); `oracle-multitenant-analyst` está en `v2.0.0` (Fase 6 — Multitenant/CDB/PDB, mismo contrato estructurado completo); `oracle-backup-recovery-analyst` está en `v1.0.0` con estructura de carpeta completa (Fase 7 — Backup & Recovery/RMAN; nunca tuvo un `agents/oracle-backup-recovery-analyst.md` plano real en Foundation — el registro apuntaba a un archivo nunca materializado, corregido en esta fase); `oracle-security-analyst` está en `v2.0.0` con estructura de carpeta completa (Fase 8 — Security & Compliance; a diferencia de RMAN, sí existía un `agents/oracle-security-analyst.md` plano real y materializado desde Foundation — su contenido (responsabilidades, boundaries, evidence policy) se preservó y amplió al deepening, nunca contradicho; el flat file fue eliminado en favor del contrato estructurado); `os-platform-analyst` está en `v2.0.0` con estructura de carpeta completa (Fase 9 — OS Platform Diagnostics & Hardening; mismo patrón que Security — sí existía un `agents/os-platform-analyst.md` plano real desde Foundation, `v1.1.0`, con contenido genuino — preservado y ampliado, flat file eliminado); `capacity-analyst` está en `v2.0.0` con estructura de carpeta completa (Fase 10 — Capacity Management & Forecasting; mismo patrón que Security/OS Platform — sí existía un `agents/capacity-analyst.md` plano real desde Foundation, `v1.0.0`, con contenido genuino — preservado y ampliado, flat file eliminado); `incident-root-cause-analyst` está en `v2.0.0` con estructura de carpeta completa (Fase 11 — Incident Analysis & Root Cause Automation; mismo patrón que Security/OS Platform/Capacity — sí existía un `agents/incident-root-cause-analyst.md` plano real desde Foundation, `v1.0.0`, con contenido genuino (Root Cause Model FACT→OBSERVATION→HYPOTHESIS→PROBABLE_CAUSE→CONFIRMED_ROOT_CAUSE→UNDETERMINED) — preservado y ampliado, flat file eliminado); `change-advisor`, `technical-documentation-manager` y `knowledge-curator` están en `v2.0.0` con estructura de carpeta completa (Fase 12 — Change Advisory, Documentation & Knowledge Lifecycle; mismo patrón que Security/OS Platform/Capacity/Incident — sí existían manifests planos reales desde Foundation (`v1.0.0`), con contenido genuino — preservado y ampliado, archivos planos eliminados; respaldados por el motor ejecutable `change_documentation_knowledge/`); el resto permanece en `v1.0.0` (Foundation, manifest plano `agents/<id>.md`) hasta que su fase correspondiente los profundice.
...
| `os-platform-analyst` | OS multiplataforma | [agents/os-platform-analyst/AGENT.md](os-platform-analyst/AGENT.md) |
| `capacity-analyst` | Capacity management/forecasting | [agents/capacity-analyst/AGENT.md](capacity-analyst/AGENT.md) |
| `incident-root-cause-analyst` | RCA/correlación multidominio | [agents/incident-root-cause-analyst/AGENT.md](incident-root-cause-analyst/AGENT.md) |
| `change-advisor` | Propuestas de cambio | [agents/change-advisor/AGENT.md](change-advisor/AGENT.md) |
| `technical-documentation-manager` | Documentación/entregables | [agents/technical-documentation-manager/AGENT.md](technical-documentation-manager/AGENT.md) |
| `knowledge-curator` | Curaduría de conocimiento | [agents/knowledge-curator/AGENT.md](knowledge-curator/AGENT.md) |
