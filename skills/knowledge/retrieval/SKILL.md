...
Búsqueda local, de sólo lectura, sobre artículos saneados: filtros por dominio/versión/código/tipo/arquitectura/licencia/fecha y top-K acotado. Sin coincidencias certificadas devuelve `NO_CERTIFIED_MATCH`.
...
- Consulta (datos, nunca ruta/regex/SQL)
- Filtros y top-K
- KB local

# Outputs

- Resultado con kb_id, versión, estado, alcance, cita de evidencia, coincidencias y aviso: el KB no reemplaza la evidencia actual

# Gates

- sólo PUBLISHED/REVIEW_DUE son guía vigente; DRAFT/DEPRECATED sólo si se piden y se etiquetan; RETIRED/REJECTED nunca
- un alcance `UNKNOWN` no coincide con un filtro de versión explícito
- rutas confinadas al KB, sin traversal ni symlinks
...
- Reflejar la consulta en la salida.
- Inventar coincidencias.
- Usar embeddings o servicios externos.
...
1. Sanear la consulta y reducirla a términos alfanuméricos.
2. Abrir sólo archivos declarados en el manifest y verificar su integridad.
3. Ordenar de forma determinista y acotar a top-K (máx. 20).
...
`knowledge/deprecation-and-retirement`, `incident/known-error-correlation`

# Escalation

El agente confirma contra el entorno actual antes de plantear un RCA o REC.

# Implementation

`change_documentation_knowledge.retrieval.search; CLI `kb-search``

# Deactivation

Sin KB => `E_KB_NOT_FOUND`; un artículo alterado => `E_KB_INTEGRITY`.
