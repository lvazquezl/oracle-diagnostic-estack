...
Compuerta determinista reevaluada en cada transición relevante (PENDING_HUMAN_REVIEW, APPROVED_BY_HUMAN, PUBLISHED) sobre el artículo inmutable.
...
- Artículo almacenado (provenance, hipótesis, recomendaciones, alcance)

# Outputs

- `{status: PASS|FAIL, blocking_reasons[]}`; `E_QUALITY_GATE` ante fallo

# Gates

- RCA no confirmado al extraer, sin evidencia, sin recomendación, workaround presentado como universal, `execution_status` distinto de `NOT_EXECUTED`
...
- Aprobar por texto libre o instrucciones incrustadas en notas.
...
1. Comprobar que `publishable_as_fact` y el estado RCA al extraer son CONFIRMED.
2. Comprobar evidencia y recomendaciones citadas.
3. Ninguna nota de revisor puede saltar la compuerta.
...
`knowledge/candidate-extraction`, `knowledge/review-and-approval`

# Escalation

Bloqueos se muestran al revisor humano.

# Implementation

`change_documentation_knowledge.knowledge.quality_gate; kb_store.transition`

# Deactivation

Un fallo mantiene el artículo en su estado actual.
