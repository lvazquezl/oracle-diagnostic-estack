---
name: change-advisory-report
id: documentation/change-advisory-report
version: 1.0.0
...
Documento derivado de un `change_advisory.json` verificado (digest + estado inmutable): propuesta manual, prerrequisitos, impacto y riesgos, rollback, validación y aprobaciones pendientes.
...
- `change_advisory.json` generado por el motor

# Outputs

- `document_change.json` / `.md` (`document_type: CHANGE_ADVISORY_REPORT`)

# Gates

- el digest del advisory debe verificarse (`E_INPUT_INVALID` si fue alterado)
- `execution_status` distinto de `NOT_EXECUTED_BY_ESTACK` se rechaza
...
- Modificar el contenido del advisory.
- Insinuar ejecución o éxito.
...
1. Verificar integridad y `execution_status`.
2. Reproducir gates, blockers, riesgos y rollback tal como están.
3. Aprobación: mostrar declaración externa (identidad no verificada) o `PENDING`.
...
`change/operational-advisory`, `documentation/change-proposal`

# Escalation

Aprobaciones pendientes se dirigen al revisor humano.

# Implementation

`change_documentation_knowledge.documents.build_change_report / validate_advisory; CLI `document --kind change``

# Deactivation

Un advisory sin cambios produce un documento `PARTIAL` con `NO_CHANGE_PROPOSALS_IN_ADVISORY`.
