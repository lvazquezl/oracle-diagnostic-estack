...
# oracle-rac-analyst. A diferencia de oracle-performance-analyst (Fase 3, listas YAML
# multi-línea), este agente declara estas 3 claves como arrays inline de una sola línea
# (`key: [a, b, c]`) — la extracción soporta ambos formatos.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
FAIL=0
R="$ROOT/agents/oracle-rac-analyst/routing.yaml"
C="$ROOT/agents/oracle-rac-analyst/collaboration.yaml"

# extract_list <file> <key> — soporta tanto "key: [a, b, c]" inline como
# "key:\n  - a\n  - b" multi-línea.
extract_list() {
  local file="$1" key="$2"
  local inline
...
# delegates_to en routing.yaml usa "  - agent: <name>" incluso en este formato; extraído aparte.
