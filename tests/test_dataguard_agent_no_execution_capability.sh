#!/usr/bin/env bash
# Valida que oracle-dataguard-analyst no declara ningún mecanismo de ejecución real.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
FAIL=0
DIR="$ROOT/agents/oracle-dataguard-analyst"
M="$DIR/manifest.yaml"

for cap in "ALTER DATABASE SWITCHOVER/FAILOVER" "DGMGRL SWITCHOVER TO" "REINSTATE DATABASE" "START OBSERVER" "ALTER DATABASE FORCE LOGGING" "ALTER DATABASE FLASHBACK"; do
