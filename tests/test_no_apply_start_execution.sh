#!/usr/bin/env bash
# Ningún artefacto ejecuta ALTER DATABASE RECOVER MANAGED STANDBY DATABASE (start).
set -uo pipefail
...
grep -q 'ALTER DATABASE RECOVER MANAGED STANDBY DATABASE' "$ROOT/agents/oracle-dataguard-analyst/manifest.yaml" \
  && echo "[PASS] manifest.yaml prohíbe explícitamente iniciar MRP" \
  || { echo "[FAIL] falta prohibición explícita"; FAIL=1; }

[ $FAIL -eq 0 ] && echo "[PASS] Ninguna query ejecuta RECOVER MANAGED STANDBY (start)"
exit $FAIL
