#!/usr/bin/env bash
# Valida el schema minimo de config/capability-matrix.yaml: 17 dominios, cada uno con las 8
...
if [ "$domain_count" -eq 17 ]; then
  echo "[PASS] config/capability-matrix.yaml declara 17 dominios"
else
  echo "[FAIL] config/capability-matrix.yaml declara $domain_count dominios, se esperaban 17"
  FAIL=1
fi
