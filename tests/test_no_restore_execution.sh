...
grep -qi '"RESTORE (cualquier variante' "$MANIFEST" && echo "[PASS] manifest prohíbe RESTORE en cualquier variante" || { echo "[FAIL] falta la prohibición de RESTORE"; FAIL=1; }
...
for f in $(find "$ROOT/agents/oracle-backup-recovery-analyst" "$ROOT/skills/rman" "$ROOT/queries/rman" "$ROOT/parsers/rman" -type f 2>/dev/null); do
  ...
  done < <(grep -niE '^\s*RESTORE\s' "$f")
done

[ $FAIL -eq 0 ] && echo "[PASS] Ninguna capacidad real de ejecutar RESTORE"
exit $FAIL
