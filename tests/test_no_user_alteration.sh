...
grep -qi "ALTER USER" "$MANIFEST" && echo "[PASS] manifest prohíbe ALTER USER" || { echo "[FAIL] falta la prohibición"; FAIL=1; }
...
for f in $(find "$ROOT/agents/oracle-security-analyst" "$ROOT/skills/security" "$ROOT/queries/security" -type f 2>/dev/null); do
  ...
  done < <(grep -niE '^\s*ALTER\s+USER\s' "$f")
done
[ $FAIL -eq 0 ] && echo "[PASS] Ninguna capacidad real de ejecutar ALTER USER"
