#!/usr/bin/env bash
# Valida que oracle-asm-storage-analyst no declara ningún mecanismo de ejecución real.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
FAIL=0
DIR="$ROOT/agents/oracle-asm-storage-analyst"
M="$DIR/manifest.yaml"

for cap in "ALTER DISKGROUP" "asmcmd write operations" "cambio de rebalance POWER" "modificación de failure groups"; do
