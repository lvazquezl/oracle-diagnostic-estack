...
grep -qi "GRANT / REVOKE" "$MANIFEST" && echo "[PASS] manifest prohíbe GRANT" || { echo "[FAIL] falta la prohibición"; FAIL=1; }

for f in $(find "$ROOT/agents/oracle-security-analyst" "$ROOT/skills/security" -type f 2>/dev/null); do
  ...
  done < <(grep -niE '^\s*GRANT\s' "$f")
done
[ $FAIL -eq 0 ] && echo "[PASS] Ninguna capacidad real de ejecutar GRANT"
...
[ $FAIL -eq 0 ] && echo "[PASS] Ninguna query certificada de Security ejecuta GRANT"
