#!/usr/bin/env bash
# Valida que oracle-network-analyst no declara ningún mecanismo de ejecución real.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
FAIL=0
DIR="$ROOT/agents/oracle-network-analyst"
M="$DIR/manifest.yaml"

for cap in "edición de listener.ora" "reload/restart de listener" "cambio de DNS" "cambio de firewall"; do
