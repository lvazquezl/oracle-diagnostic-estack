...
# Sobre 11g genérico (11.2, alineado con tests/test_query_variant_resolver_11g.sh)
...
[ "$found_v1" -eq 1 ] && echo "[PASS] variante legacy (10.2-12.1) resuelve para 11g (11.2)" || { echo "[FAIL] variante legacy no resuelve para 11g"; FAIL=1; }
[ "$found_v2" -eq 0 ] && echo "[PASS] variante moderna NO resuelve para 11g (11.2) — V\$DATAGUARD_PROCESS no existe antes de 12.2.0.1, sin fallback silencioso (# 10)" || { echo "[FAIL] la variante moderna resolvió para 11g — no debería, la vista no existe en esa versión"; FAIL=1; }
