...
# oracle-backup-recovery-analyst. Mismo patrón dual-formato que test_multitenant_agent_no_delegation_loop.sh.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
FAIL=0
R="$ROOT/agents/oracle-backup-recovery-analyst/routing.yaml"
C="$ROOT/agents/oracle-backup-recovery-analyst/collaboration.yaml"
