#!/usr/bin/env bash
# Ningún artefacto declara EDIT DATABASE/EDIT CONFIGURATION ejecutable (# 24).
set -uo pipefail
...
for f in "$ROOT"/parsers/dataguard/*.py "$ROOT/docs/DATAGUARD_BROKER_READONLY_COLLECTORS.md" "$ROOT/mcp/tool-manifest.md"; do
  ...
  done < <(grep -niE 'EDIT (DATABASE|CONFIGURATION)' "$f")
done

[ $FAIL -eq 0 ] && echo "[PASS] Ningún artefacto declara EDIT DATABASE/CONFIGURATION ejecutable"
exit $FAIL
