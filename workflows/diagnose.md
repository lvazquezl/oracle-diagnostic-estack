...
Cualquiera del registro, según la clasificación del síntoma por el orquestador (ej. "lento" → `oracle-performance-analyst`; "no conecta" → `oracle-network-analyst`; "espacio" → `oracle-dba-analyst`/`capacity-analyst`; "¿cuándo se llena?"/"crecimiento" → `capacity-analyst`). Escenarios Fase 4 (`# 43` del prompt de Fase 4): `rac` → `rac/troubleshooting`; `service`/`scan`/`listener`/`connection` → `network/troubleshooting`/`rac/troubleshooting` según si el síntoma es de conectividad o de recurso Clusterware; `interconnect` → `rac/interconnect`/`network/interconnect`; `asm` → `asm/troubleshooting`. Escenarios Fase 5 (`# 48` del prompt de Fase 5): `dataguard`/`transport`/`apply`/`lag`/`gap`/`broker`/`fsfo`/`switchover-readiness`/`failover-readiness` → `dataguard/troubleshooting`, que enruta al skill Data Guard específico. Escenarios Fase 6 (`# 43` del prompt de Fase 6): `cdb`/`pdb`/`pdb-open-state`/`pdb-service`/`pdb-temp`/`pdb-undo`/`plugin-violation`/`pdb-resource`/`pdb-rac-placement` → `multitenant/troubleshooting`, que enruta al skill Multitenant específico. Escenarios Fase 7 (`# 39` del prompt de Fase 7): `rman`/`backup-failure`/`backup-slow`/`channel-contention`/`fra-pressure`/`archivelog-backup-gap`/`controlfile-backup`/`snapshot-controlfile`/`sbt-media-manager`/`restore-readiness`/`recovery-readiness` → `rman/troubleshooting`, que enruta al skill RMAN específico (`rman/channel-contention`, `rman/fra-pressure`, `rman/archivelog-backup`, `rman/controlfile-backup`, `rman/snapshot-controlfile`, `rman/sbt-media-manager`, `rman/restore-readiness`, `rman/recovery-readiness` según el síntoma). Escenarios Fase 8 (`# 51` del prompt de Fase 8): `security`/`privilege-exposure`/`public-grants`/`account-posture`/`password-policy`/`password-profile`/`audit-gap`/`tde-posture`/`network-encryption`/`compliance-gap` → enrutan al skill Security específico (`security/powerful-privileges`, `security/public-grants`, `security/account-inventory`, `security/password-policy-strength`, `security/password-profiles`, `security/audit-configuration`, `security/tde-awareness`, `security/network-encryption`, `security/compliance-mapping` según el síntoma). Escenarios Fase 9 (OS Platform): `memory-pressure`/`swap-pressure`/`hugepages`/`thp`/`open-files`/`process-limits`/`semaphore`/`shared-memory`/`aio`/`ephemeral-ports`/`filesystem-full`/`inode-pressure`/`io-latency`/`bonding`/`vlan`/`mtu`/`time-sync`/`oracle-os-limits` → activan `os-platform-analyst`, que enruta internamente al skill `os/*` específico del síntoma (`os/memory-pressure`, `os/swap`, `os/hugepages`, `os/transparent-hugepages`, `os/open-files`, `os/process-limits`, `os/semaphores`, `os/shared-memory`, `os/aio`, `os/ephemeral-ports`, `os/filesystems`, `os/inodes`, `os/io-performance`, `os/bonding`, `os/vlan`, `os/mtu`, `os/time-sync`, `os/kernel-parameter-assessment`) — deliberadamente **no** un comando slash por cada `sysctl`/parámetro de kernel individual, el síntoma reportado por el DBA en lenguaje natural es lo que enruta, igual que el resto de dominios. Escenarios Fase 10 (Capacity Management & Forecasting): `capacity`/`capacity-cpu`/`capacity-memory`/`capacity-storage`/`capacity-asm`/`capacity-tablespace`/`capacity-vmware`/`capacity-sqlserver`/`capacity-source-conflict`/`capacity-data-quality`/`capacity-threshold`/`running-out-of-space`/`growth-forecast`/`when-will-we-run-out` → activan `capacity-analyst`, que enruta internamente al skill `capacity/*` específico del síntoma (`capacity/cpu`, `capacity/memory`, `capacity/storage`, `capacity/asm`, `capacity/tablespace`, `capacity/vmware`, `capacity/sqlserver`, `capacity/data-source-inventory` cuando el síntoma es `capacity-source-conflict`, `capacity/data-quality`, `capacity/threshold-crossing`, `capacity/forecasting` cuando el síntoma es una pregunta de proyección temporal) — un solo agente transversal, nunca un especialista por tecnología (`# 20`-`# 40` del prompt de Fase 10: "un único agente capacity-analyst, nunca dividido por recurso/tecnología"). Escenarios Fase 11 (Incident Analysis & Root Cause Automation): `incident`/`root-cause`/`why-did-this-happen`/`recurring-issue`/`post-mortem`/`what-caused-this` → activan `incident-root-cause-analyst` (`/diagnose incident`), que enruta internamente por `incident/classification` al conjunto de skills de correlación cross-domain relevantes al síntoma (mismo patrón que el resto de dominios: el síntoma reportado por el DBA en lenguaje natural es lo que enruta, nunca un comando por dominio individual). A diferencia de los demás escenarios de `/diagnose` (que activan un único especialista), un síntoma clasificado como "incident" activa el flujo completo de `workflows/incident.md`/`workflows/rca.md` según si el incidente está activo o ya cerrado. No se crea un comando slash por cada código ORA/TNS/CRS/RMAN individual — la taxonomía vive en `knowledge/errors/{ora,tns,crs,rman}/`.
