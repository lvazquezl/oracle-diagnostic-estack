...
- Incident → activa `incident-root-cause-analyst` sólo vía `/healthcheck incident` explícito o cuando otro workflow escala (`workflows/diagnose.md#stop-conditions`, `workflows/incident.md`). `incident-root-cause-analyst` nunca vuelve a recolectar evidencia de dominio por su cuenta — consume por referencia la de los 10 especialistas previos vía sus skills de correlación cross-domain, activando cada uno sólo si la hipótesis en curso lo requiere.
...
gates:
  ...
  incident:     incident-root-cause-analyst nunca duplica collectors de ninguna fase previa — sólo consume evidence_refs por referencia vía sus skills de correlación cross-domain; ninguna acción de remediación se ejecuta, siempre manual_remediation_plan con execution_status NOT_EXECUTED
```
